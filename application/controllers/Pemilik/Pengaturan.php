...
class Pengaturan extends CI_Controller {
    ...
    public function index() {
        // Gunakan id_user = 2 (hardcoded sesuai permintaan)
        $id_user = 2;
        ...
        if (!$user) {
            show_error('User dengan ID 2 tidak ditemukan');
            return;
        }

        // Ambil data pemilik berdasarkan id_user = 2
        $pemilik = $this->db->where('id_user', $id_user)
                            ->where('deleted_at IS NULL')
                            ->get('pemilik')
                            ->row();

        // Jika tidak ada data pemilik, buat baru
        if (!$pemilik) {
        ...
        $branches = $this->db->where('deleted_at IS NULL')->get('cabang')->result() ?: [];
    ...
    public function change_password() {
        ...
        $user = $this->User_model->getUserById($id_user);
        if (!$user) {
            echo json_encode(['success' => false, 'message' => 'User dengan ID 2 tidak ditemukan di database']);
            return;
        }

        log_message('debug', "change_password - user password hash: " . substr($user->password, 0, 20) . "...");
        log_message('debug', "change_password - password_lama input: $password_lama");
    ...
    public function add_employee() {
        ...
        $base_username = strtolower(preg_replace('/[^a-zA-Z0-9]/', '_', $nama));
        $username = $base_username;
        $counter = 1;
        
        // Cek apakah username sudah ada, jika ada tambahkan counter
        while ($this->User_model->checkUsername($username)) {
            $username = $base_username . '_' . $counter;
            $counter++;
        }
        
        $password = bin2hex(random_bytes(4)); // 8 karakter

        log_message('debug', "add_employee - Generated username: $username, password: $password");
        ...
        if (!$this->User_model->insertUser($user_data)) {
        ...
        log_message('debug', "add_employee - User berhasil dibuat dengan id_user: $id_user");

        // Insert ke tabel karyawan (tanpa id_pemilik karena tidak ada di tabel)
        $karyawan_data = [
            'id_user' => $id_user,
            'id_cabang' => $id_cabang,
            'nama' => $nama,
    ...
    public function add_branch() {
        $id_pemilik = (int) $this->input->post('id_pemilik');
        ...
        // Jika id_pemilik tidak ada, gunakan pemilik pertama
        if ($id_pemilik === 0) {
            $first_pemilik = $this->db->where('deleted_at IS NULL')->get('pemilik')->row();
            if ($first_pemilik) {
                $id_pemilik = $first_pemilik->id_pemilik;
            } else {
                echo json_encode(['success' => false, 'message' => 'Data pemilik tidak ditemukan di database']);
                return;
            }
        } else {
